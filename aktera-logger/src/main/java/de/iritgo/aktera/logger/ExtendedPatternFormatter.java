...
public class ExtendedPatternFormatter extends PatternFormatter
{
	...
	private String getMethod(final LogEvent event)
	{
		...
		Class<?> clazz = StackIntrospector.getCallerClass(de.iritgo.aktera.logger.Logger.class, 0);

		if (clazz != null && clazz.getName().contains("GroovyReflector"))
		{
			clazz = StackIntrospector.getCallerClass(de.iritgo.aktera.logger.Logger.class, 6);
		}
		else
		{
			clazz = StackIntrospector.getCallerClass(de.iritgo.aktera.logger.Logger.class, - 1);
		}
