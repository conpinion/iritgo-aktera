...
public class ImportManagerImpl implements ImportManager
{
	...
	public void initialize ()
	{
		...
		for (Configuration config : sortedConfigs)
		{
			try
			{
				...
				ImportHandlerConfig ihc = new ImportHandlerConfig (config.getAttribute ("id"), config
								.getAttribute ("root"), handler);

				importHandlerConfigs.add (ihc);
			}
			catch (Exception x)
			{
				logger.error ("Unable to read import handler configuration '" + config.getAttribute ("id", "?") + "'",
								x);
			}
		}
		...
		for (Configuration config : configuration.getChildren ("csvImport"))
		{
			try
			{
				CsvImportHandlerConfig cihc = new CsvImportHandlerConfig (config.getAttribute ("id"), config
								.getAttribute ("xsl"));
			...
			{
				logger.error ("Unable to read csv import handler configuration '" + config.getAttribute ("id", "?")
								+ "'", x);
			}
		}
	}
			...
			reader.setContentHandler (new DefaultHandler ()
			{
				...
				public void endElement (String uri, String localName, String name) throws SAXException
				{
					...
					if (currentHandler != null)
					{
						...
						if (! allwhite)
						{
							currentHandler.elementContent (uri, localName, name, StringTools.trim (tagContent
											.toString ()), reporter, properties);
						}
					...
					String handlerId, Properties properties) throws ModelException
	{
		...
		for (ImportHandlerConfig importHandlerConfig : im.getImportHandlerConfigs ())
		{
			...
			{
				res = res
								&& importHandlerConfig.getHandler ().analyze (req, doc, importElem, reporter, i18n,
												properties);
			}
			catch (Exception x)
			{
					...
					String handlerId, Properties properties) throws ModelException
	{
		...
		for (ImportHandlerConfig importHandlerConfig : im.getImportHandlerConfigs ())
		{
			...
			{
				res = res
								&& importHandlerConfig.getHandler ().perform (req, doc, importElem, reporter, i18n,
												properties);
			}
			catch (Exception x)
			{
