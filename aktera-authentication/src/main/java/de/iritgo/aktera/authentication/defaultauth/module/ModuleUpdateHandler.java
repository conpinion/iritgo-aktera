...
public class ModuleUpdateHandler extends UpdateHandler
{
					...
					ModuleVersion currentVersion, ModuleVersion newVersion) throws Exception
	{
		...
		if (currentVersion.lessThan("2.3.1"))
		{
			...
			{
				insert("AkteraGroup", "name", "'" + AkteraGroup.GROUP_NAME_MANAGER + "'", "protect", "true", "visible",
								"false", "title", "'$Aktera:managers'");
			...
			if (count("AkteraGroup", "name = '" + AkteraGroup.GROUP_NAME_USER + "'") == 0)
			{
				...
				if (userGroupId != - 1)
				{
					...
					{
						insert("AkteraGroupEntry", "groupId", String.valueOf(userGroupId), "userId", String.valueOf(row
										.get("uniqid")), "position", String.valueOf(position++));
					}
				}
			}
