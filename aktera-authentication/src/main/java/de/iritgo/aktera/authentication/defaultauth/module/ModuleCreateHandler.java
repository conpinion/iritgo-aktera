...
public class ModuleCreateHandler extends CreateHandler
{
	...
	{
		createTable("AkteraGroup", "id serial primary key", "name varchar(255) not null", "protect boolean",
						"title varchar(255)", "visible boolean");

		createTable("AkteraGroupEntry", "groupId int4 not null", "id serial primary key", "position int4 not null",
						"userId int4 not null");

		createTable("keelgroupmembers", "GroupName varchar(80) not null", "UniqId int4 not null");

		createTable("keelusers", "email varchar(132)", "ldapName varchar(120)", "userName varchar(80) not null",
						"passwd varchar(255)", "uniqid serial primary key");
					...
					ModelRequest request) throws ModelException, PersistenceException, SQLException
	{
		...
		createInstanceSecurity("de.iritgo.aktera.persist.defaultpersist.DefaultPersistent", "keel.groupmembers",
						"root", "*");
		createInstanceSecurity("de.iritgo.aktera.persist.defaultpersist.DefaultPersistent", "keel.user", "anonymous",
						"L");
		createInstanceSecurity("de.iritgo.aktera.persist.defaultpersist.DefaultPersistent", "keel.group", "anonymous",
						"L");
		createInstanceSecurity("de.iritgo.aktera.persist.defaultpersist.DefaultPersistent", "keel.groupmembers",
						"anonymous", "L");
		...
		update("INSERT INTO keelusers (uniqid, userName, email, passwd)"
						+ " values (0, 'anonymous', 'anonymous@unknown', '" + StringTools.digest("") + "')");
		update("INSERT INTO keelusers (uniqid, userName, email, passwd)" + " values (1, 'admin', 'admin@unknown', '"
						+ StringTools.digest("admin") + "')");
		update("INSERT INTO keelusers (uniqid, userName, email, passwd)"
						+ " values (2, 'manager', 'manager@unknown', '" + StringTools.digest("manager") + "')");
		...
		update("INSERT INTO AkteraGroup (name, protect, title, visible) values ('"
						+ AkteraGroup.GROUP_NAME_ADMINISTRATOR + "', true, '$Aktera:administrators', false)");
